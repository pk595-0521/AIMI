...
await candidate.reload();await candidate.getByRole('button',{name:'Start Assessment'}).click();await candidate.getByRole('heading',{name:'Before you begin'}).waitFor();const me=await api(candidate,'/me');console.log('Live governanceReady='+me.data.governanceReady);
if(me.data.governanceReady){for(const box of await candidate.getByRole('checkbox').all())await box.check();await candidate.getByRole('button',{name:'Accept and enter assessment'}).click();await candidate.getByRole('button',{name:'Complete segment 1'}).waitFor();console.log('PASS live candidate workspace');}
await grader.reload();const queue=(await api(grader,'/grader/sessions')).data;const session=queue.find(s=>s.applicant.email===c.email);assert.ok(session);await grader.getByLabel('Assigned assessment').selectOption(session.id);await grader.getByRole('heading',{name:'Observable signals'}).waitFor();console.log('PASS live grader queue and evidence view');
