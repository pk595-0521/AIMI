...
4. Provision an Organization and User records mapped to the OIDC provider's immutable `sub`. Roles come from the database, never browser inputs or token role claims. Provision certified graders and assess their training independently; the software cannot certify competence.
...
7. Put a trusted OIDC gateway and HTTPS reverse proxy in front of the server. The gateway must complete login and refresh, and set the `__Host-aimi_token` cookie with `Secure; HttpOnly; SameSite=Strict; Path=/` and no Domain. It must carry a valid signed JWT with issuer, audience, subject and expiry; RS256 and ES256 are accepted. The API also accepts bearer JWTs. Browser writes require exact APP_ORIGIN. No development auth bypass exists.
