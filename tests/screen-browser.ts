...
try{
 for(const t of SCREEN_TRACKS){
  ...
  await page.setViewportSize({width:390,height:844});const submitBox=await page.getByRole('button',{name:'Submit Session',exact:true}).boundingBox();assert.ok(submitBox&&submitBox.x>=0&&submitBox.x+submitBox.width<=390);assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.screenshot({path:`tests/artifacts/screen-${t.id}-mobile.png`});
