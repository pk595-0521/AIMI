...
if(process.env.LIVE_SMOKE_GROQ==='true'){
 const chat=candidate.getByPlaceholder('Ask Copilot for analysis, formulas, or risk frameworks…');await chat.fill('What is Vantage Logistics Solutions annual revenue? Provide a Markdown comparison table with four columns: Company, Annual revenue, Evidence, and Limitation. Use only released case facts.');await chat.press('Enter');
