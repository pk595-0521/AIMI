...
export function sessionHygiene(s:any) {
  if(s.scenarioSnapshot.assessmentType==='AIMI_SCREEN'){const last=(s.hygieneEvents||[]).filter((e:any)=>e.type==='SCREEN_HYGIENE_ATTEMPT').at(-1);return {raw_hygiene_score:last?.raw_hygiene_score||null,hygiene_multiplier:last?.hygiene_multiplier??0};}
