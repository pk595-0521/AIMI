...
for(const track of TRACK_LIST){
 ...
 await page.goto(base);await page.getByRole('button',{name:'Launch Simulation'}).click();await page.getByRole('heading',{name:'Before you begin'}).waitFor();assert.equal(await page.getByRole('button',{name:'Accept and enter assessment'}).isEnabled(),false);
 ...
 await page.reload();await page.getByRole('button',{name:'Launch Simulation'}).click();await page.getByRole('button',{name:'Audit logs',exact:true}).click();assert.equal(await page.getByLabel('Assumption',{exact:true}).inputValue(),'Hypothesis persisted through reload');
 ...
 await page.reload();await page.getByRole('button',{name:'Launch Simulation'}).click();await page.getByRole('button',{name:'Stakeholder inbox',exact:true}).click();await page.getByRole('heading',{name:track.emergencyConstraint.title,exact:true}).waitFor();
