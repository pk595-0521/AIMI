...
export async function refreshScreenClock(tx:any,s:any):Promise<any>{
 ...
 if(phase===s.activePhase&&!expired)return s;
 const data:any={activePhase:phase,revision:{increment:1}};
...
export function assertScreenAnalysis(s:any){if(isScreen(s)&&(!s.dataHandling||s.activePhase<2))throw new HttpError(403,'Pass the data hygiene gate and wait until minute 5 before analysis or Copilot use.');}
export async function syncScreen(tx:any,s:any,input:any){
 const data:any={revision:{increment:1}};
 if(input.action==='data-handling'){
  if(s.dataHandling)throw new HttpError(409,'The gate has already passed.');
  ...
  data.hygieneEvents=[...(s.hygieneEvents||[]),{type:'SCREEN_HYGIENE_ATTEMPT',phase:s.activePhase,at:new Date().toISOString(),decisions:input.dataHandling,passed,...score}];
  if(passed)data.dataHandling=input.dataHandling;
