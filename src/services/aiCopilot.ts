...
export async function queryCopilot(options: CopilotQueryOptions, onDelta?:(text:string)=>void): Promise<{reply:string;logId:string}> {
  ...
  for await(const data of sseData(response.body)){const event=JSON.parse(data);if(event.type==='error')throw new Error(event.error);if(event.logId)logId=event.logId;if(event.type==='delta'){reply+=event.text;onDelta?.(reply);}if(event.type==='done')complete=true;}
